"""Evaluate a wapic model on the structured bad-case eval set (bcv3, 500 cases).

Historical baselines: old release wapic-20260605 = 413/500.
...
Usage: python3 scripts/badcase.py MODEL [--input data/badcase_eval_v3.jsonl]
"""
import argparse
import json
import os
...
def wapic_cut_batch(model, srcs):
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False, suffix=".in") as fin:
        ...
        ip = fin.name
    op = ip + ".out"
    subprocess.run([os.environ.get("WAPIC_BIN", "./build/wapic"), "test", "-m", model, ip, op],
...
def main():
    ...
    ap.add_argument("--input", default="data/badcase_eval_v3.jsonl")
