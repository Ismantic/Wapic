#!/usr/bin/env bash
# H18.2: H17.2 mix + decimal_focus (18.7k sentences with digit.digit+quantifier)
# Target: fix "1.25亿" style decimal-split cut errors.
set -e
cd "$(dirname "$0")/.."
...
source experiments/run_batch_h17.sh.lib 2>/dev/null || true
...
echo "=== H18.2: H17.2 + decimal_focus 18.7k ===" | tee -a experiments/results.log
date | tee -a experiments/results.log

NAME=h18_2_h172_PLUS_decfocus_i50
cat data/opennews_nh_2m_train.txt data/clean_aug_train.txt data/pd_mp_train.txt data/pd_mp_train.txt data/antimerge_train.txt data/anchor_1m.txt data/threechar_zh_1m_train.txt data/decimal_focus_train.txt > data/h18_2.txt
./build/wapic fit $COMMON -i 50 data/h18_2.txt data/exp_${NAME}.wac > experiments/log_${NAME}.txt 2>&1
eval_full data/exp_${NAME}.wac
eval_pruned data/exp_${NAME}.wac 0.15

echo "===H18.2 DONE===" | tee -a experiments/results.log
date | tee -a experiments/results.log
